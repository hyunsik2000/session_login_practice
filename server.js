...
app.use(
  session({
    secret: "session secret", // 암호화, 열쇠 역할을 하는 문자열 설정
...
app.post("/", (req, res) => {
  const { userId, userPassword } = req.body; // 2️⃣. 요청 바디에서 전달받은 값을 구조분해 할당을 사용하여 관리하세요.
...
// GET 요청
app.get("/", (req, res) => {
  const userInfo = users.find((el) => el.user_id === req.session.userId); // json 형식으로 내보내기
  return res.json(userInfo);
});

// DELETE 요청
app.delete("/", (req, res) => {
  req.session.destroy(); // 4️⃣. 세션 내 정보를 삭제하는 메소드를 작성하세요.
